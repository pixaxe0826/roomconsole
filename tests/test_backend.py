...
def test_widget_data_reload(setup):
 _,c=setup;assert len(c.post('/api/widgets/reload').json()['widgets'])==8;assert c.put('/api/widgets/note/data',json={'data':{'text':'hello'}}).status_code==200;assert c.get('/api/state').json()['widget_data']['note']['text']=='hello';assert c.put('/api/widgets/nope/data',json={'data':{}}).status_code==404
