...
from benchmarks.dataset import DEFAULT_ROOT, load_suite, stable
...
@pytest.fixture
def suite():
    return load_suite('room_hub_v1')


@pytest.fixture
def copied_suite(tmp_path):
    target = tmp_path / 'suites' / 'custom'
    shutil.copytree(DEFAULT_ROOT / 'room_hub_v1', target)
    return target


def edit_cases(path, fn):
    p = path / 'all_250.jsonl'
...
def test_original_suite_counts_and_gold_files(suite):
    assert len(suite.cases) == 250
    assert len(suite.select(source_type='unique')) == 100
    assert len(suite.select(source_type='paraphrase')) == 150
    assert len({c['expected']['capability'] for c in suite.cases}) == 52
...
@pytest.mark.parametrize('mutation', [
    ...
    lambda r: r[100].update(parent_id='not-present'),
    ...
    lambda r: r[55]['expected']['slots'].update(memo_id='not-in-fixture'),
...
def test_duplicate_json_keys(copied_suite):
    p = copied_suite / 'all_250.jsonl'
    p.write_text(p.read_text().replace('"id":"B001"', '"id":"B001","id":"B001"', 1)) if '"id":"B001"' in p.read_text() else p.write_text(p.read_text().replace('"id": "B001"', '"id": "B001", "id": "B001"', 1))
    with pytest.raises(ValueError): load_suite('custom', copied_suite.parent)
...
def test_no_corpus_utterances_or_gold_imports_in_runtime():
    ...
    assert 'B001' not in text and 'all_250' not in text
...
def test_small_external_suite_add_replace_remove_without_code(copied_suite):
    original = [json.loads(x) for x in (copied_suite/'all_250.jsonl').read_text().splitlines()]
...
def test_jsonl_publishing_is_limited_to_suite_sources():
    from scripts.repo_policy import source_path_allowed
    assert source_path_allowed('benchmarks/suites/example/cases.jsonl')
