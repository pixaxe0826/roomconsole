"""Verified source-only patch installer. Never imports the app; reads only two STT settings for preservation checks.
...
PATCH_ID='room-hub-0.1.6-grounded-context1'
ROOT_FILES={'START_HERE.html','.dockerignore','THIRD_PARTY_NOTICES.md','.gitignore','VERSION','README.md','README_KO.md','CHANGELOG.md','AGENTS.md','SECURITY.md','run.py','compose.yaml'}
ROOT_DIRS={'.github','app','web','widgets','docs','scripts','tests','previews','deploy'}
DENY={'data','.git','.venv','.venv-v35','.termux','backups','secrets','node_modules','runtime','models'}
PROTECTED={'app/speech.py','app/stt_accuracy.py','web/client.html','web/client.js','web/client.css','web/speech-client.js','web/speech.js','web/speech.css'}
...
def safe_path(root:Path,relative:str)->Path:
 if relative in PROTECTED:raise ValueError('Protected STT/client path: '+relative)
...
def speech_guard(target:Path)->dict:
 """Read known settings only. Never changes them, reads models or shows keys.

 For an offline source-only checkout (no settings at all) return an explicit
 unavailable status. Operational installs require the existing Base/ko/6T and
 careful/beam5 profile. HUB_DATA_DIR must match the actual server if customized.
 """
 value=os.environ.get('HUB_DATA_DIR')
 data=Path(value).expanduser() if value else target/'data'
 if not data.is_absolute():data=target/data
 paths=[data/'speech-config.json',data/'stt-accuracy.json']
 if not any(p.exists() for p in paths):return {'checked':False,'reason':'source-only checkout; no STT settings present'}
 for p in paths:
  if p.is_symlink() or not p.is_file() or p.stat().st_size>32768:raise ValueError('Missing/invalid protected STT settings: '+p.name)
 raw=[p.read_bytes() for p in paths];cfg,accuracy=[json.loads(b) for b in raw]
 if (not isinstance(cfg,dict) or not isinstance(accuracy,dict)
     or Path(str(cfg.get('model',''))).name!='ggml-base.bin' or cfg.get('language')!='ko'
     or type(cfg.get('threads')) is not int or cfg['threads']!=6
     or accuracy.get('profile')!='careful'):
  raise ValueError('Expected existing Whisper base / ko / 6T / careful(beam5). Settings were NOT changed; check stt-accuracy.sh status --verify first.')
 return {'checked':True,'model':'ggml-base.bin','threads':6,'language':'ko','beam':5,
         'sha256':{p.name:digest(b) for p,b in zip(paths,raw)}}
...
def plan_update(package:Path,target:Path)->tuple[dict,list]:
 ...
 speech_guard(target)
 version=(target/'VERSION').read_text('utf-8').strip() if (target/'VERSION').exists() else ''
 if version not in {'0.1.5','0.1.6'}:raise ValueError('This patch requires an existing 0.1.5 installation (or already-applied 0.1.6). No files changed.')
 ...
 for entry in manifest['files']:
  ...
  if not exists and entry.get('optional_existing'):continue
...
def apply_update(package:Path,target:Path,backup_parent:Path|None=None)->Path|None:
 ...
 record={'patch_id':PATCH_ID,'target':str(target.resolve()),'files':plan,'protected_stt':protected,'status':'prepared'}
 ...
 try:
  ...
  if speech_guard(target)!=protected:raise ValueError('STT configuration changed during update; source update rolled back, settings were not touched.')
...
def main()->int:
 ...
 try:
  ...
  if args.check:
   print('READY: '+str(len(plan))+' source files to update. No files changed.');print('STT:',json.dumps(speech_guard(target),ensure_ascii=False));return 0
  ...
  print('APPLIED: '+PATCH_ID+'\nBACKUP: '+str(backup)+'\nStart Room Hub, check /healthz for 0.1.6, then reload Manager and Client. New auto mode uses reads and explicitly confirmed writes. No models or inference settings were changed.\nPrivate data, sessions, keys, layout, services and wake-lock settings were not changed.')
  return 0
