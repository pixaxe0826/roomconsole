  ...
  try:
   with httpx.Client(base_url=url,trust_env=False,headers={'Authorization':'Bearer '+app.state.admin_token,'X-Room-Request':'1'}) as api:
    ...
    with sync_playwright() as p:
     ...
     page.wait_for_function("document.querySelector('#assistantCatalog').textContent.includes('todo.list')")
     ...
     with httpx.Client(base_url=url,trust_env=False,headers={'X-Room-Request':'1'}) as display:
      ...
      tablet.wait_for_function("document.querySelector('[data-llmr-output]')?.textContent.includes('내일 택배')")
      ...
      tablet.wait_for_function("document.querySelector('[data-llmr-output]')?.textContent.includes('답변을 채택하지')")
