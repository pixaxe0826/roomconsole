...
Usage: python scripts/assistant_patch_regression.py --package /path/to/patch --baseline /path/to/0.1.4
"""
...
def main():
 ...
 result={'scope':'Temporary copy of actual 0.1.4 source; real SQLite, TestClient API and patch installer. Synthetic records, no user files/services.','checks':[]}
 def check(name,ok):
 ...
 with tempfile.TemporaryDirectory() as tmp:
  ...
  # Original 0.1.4 session/task/settings, made by the original code before applying.
  run(target,r'''
import json
from pathlib import Path
...
''')
  protected={'data/speech-config.json':b'{"model":"base","threads":6}', 'data/llm-protected-test.txt':b'not-a-real-key', '.env':b'# synthetic env unchanged', 'runtime/qwen/build/bin/llama-server':b'fixture-not-executable', 'data/https/private/fixture.key':b'not-a-real-private-key'}
  ...
  _,plan=u.plan_update(args.package,target);check('0.1.4 current source accepted',len(plan)>0)
  backups=u.apply_update(args.package,target)
  check('new version installed',(target/'VERSION').read_text().strip()=='0.1.5')
  ...
  (target/'data/speech-config.json').unlink()
  run(target,r'''
import json,time
from pathlib import Path
from fastapi.testclient import TestClient
...
with TestClient(app) as c:
 ...
 s=c.get('/api/state').json();assert s['tasks'][0]['title']=='기존 보존 항목';assert s['settings']==saved['settings'];assert s['layout']==saved['layout']
...
''')
  ...
  u.rollback(target,backups);check('code rollback restores 0.1.4',(target/'VERSION').read_text().strip()=='0.1.4')
  check('new module removed on rollback',not (target/'app/assistant.py').exists())
  run(target,r'''
from pathlib import Path
from fastapi.testclient import TestClient
from app.main import create_app
...
''')
  ...
  (target/'app/assistant.py').symlink_to(target/'app/main.py')
  try:u.plan_update(args.package,target);ok=False
  except ValueError:ok=True
  check('symlink payload target rejected',ok)
  (target/'app/assistant.py').unlink()
  check('new optional Qwen preflight skipped if not installed',not (target/'deploy/termux/qwen-runtime/preflight.py').exists())
 result['passed']=len(result['checks'])
 out=Path(__file__).resolve().parents[1]/'artifacts/test-results/assistant-patch.json';out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(result,ensure_ascii=False,indent=2))
