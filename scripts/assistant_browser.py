    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with httpx.Client(base_url=base,trust_env=False,headers={'Authorization':'Bearer '+app.state.admin_token,'X-Room-Request':'1'}) as api:
            ...
            with sync_playwright() as p:
                ...
                tablet.wait_for_function("document.querySelector('[data-llmr-output]')?.textContent.includes('추가했습니다')")
                ok('injected invalidation fetches real confirmed result for tablet', '추가했습니다' in tablet.locator('[data-llmr-output]').inner_text())
                page.locator('[data-llm=retry]').click();page.wait_for_function("document.querySelector('#llmDetail')?.textContent.includes('이미 실행한 작업')")
                ...
                page.locator('.assistant-confirm-box [data-llm=cancel]').click();page.wait_for_function("document.querySelector('.llm-detail-heading')?.textContent.includes('취소됨')")
