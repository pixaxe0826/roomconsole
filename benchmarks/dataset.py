...
from .paths import NAME, external_directory, owned_child

DEFAULT_ROOT = None  # Never fall back to a repository-bundled evaluation suite.
...
def child(root: Path, name: str) -> Path:
    return owned_child(root, name)


def suite_root(root=None) -> Path:
    return external_directory(root if root is not None else os.environ.get('ROOM_HUB_BENCHMARK_DATA'))


def suite_file_names(manifest: dict, path: Path) -> list[str]:
    """Explicit manifest references; legacy manifest fallback is unambiguous only."""
    files = manifest.get('case_files')
    if files is None:
        # Original V1 export uses all_250.jsonl; no sentences or answers are embedded.
        candidates = [n for n in ('cases.jsonl', 'all_250.jsonl', 'cases.json') if (path / n).is_file()]
        if len(candidates) != 1:
            raise ValueError('Set manifest.case_files explicitly (cannot resolve case files uniquely)')
        files = candidates
    if not isinstance(files, list) or not files or not all(isinstance(f, str) for f in files):
        raise ValueError('case_files must be a nonempty string list')
    return files


def referenced_files(manifest: dict, path: Path) -> list[str]:
    names = ['manifest.json', manifest.get('schema_file', 'schema.json'),
             manifest.get('fixtures_file', 'fixtures.json'), *suite_file_names(manifest, path)]
    names += [manifest[k] for k in ('projection_file', 'registry_file') if manifest.get(k)]
    return names


def aware(value: str) -> datetime:
    if not isinstance(value, str):
        raise ValueError('reference_datetime must be a string')
...
def load_suite(name: str, root=DEFAULT_ROOT) -> Suite:
    ...
    root = suite_root(root)
    return load_directory(root / name, name=name)


def load_directory(path: Path, *, name: str | None = None) -> Suite:
    path = external_directory(path)
    manifest = json_read(child(path, 'manifest.json'))
    if not isinstance(manifest, dict):
        raise ValueError('Manifest must be an object')
    name = name or manifest.get('suite') or path.name
    if not isinstance(name, str) or not NAME.fullmatch(name):
        raise ValueError('Invalid suite name')
    if manifest.get('suite') and manifest['suite'] != name:
        raise ValueError('Manifest suite name differs from selected suite directory')
    from .integrity import RECEIPT, verify_inventory
    receipt = json_read(child(path, RECEIPT)) if (path / RECEIPT).exists() else None
    if receipt is not None:
        verify_inventory(path, receipt)
        if receipt['suite'] != name:
            raise ValueError('Transfer receipt suite mismatch')
    ...
    files = suite_file_names(manifest, path)
    if not files or not isinstance(files, list):
        raise ValueError('case_files must be a nonempty list')
    from .integrity import MAX_FILES, MAX_TOTAL
    names = referenced_files(manifest, path)
    if len(names) > MAX_FILES or len(set(names)) != len(names):
        raise ValueError('Duplicate or excessive referenced files')
    if sum(child(path, n).stat().st_size for n in names) > MAX_TOTAL:
        raise ValueError('Suite exceeds 128 MiB')
    for label, document in [('schema', schema), ('fixtures', fixtures), ('projection', projection)]:
        if not isinstance(document, dict):
            raise ValueError(label + ' must be an object')
    if not isinstance(schema.get('slot_schemas', {}), dict):
        raise ValueError('slot_schemas must be an object')
    rows = []
    for filename in files:
        ...
        if p.suffix.lower() == '.json':
            array = json_read(p)
            if not isinstance(array, list) or not all(isinstance(r, dict) for r in array):
                raise ValueError('JSON case file must contain an array of objects')
            rows.extend(array)
            continue
        if p.suffix.lower() != '.jsonl':
            raise ValueError('case_files supports .json arrays or .jsonl only')
    ...
    if registry_file:
        ...
        if not isinstance(registry, dict) or not isinstance(registry.get('counts'), dict):
            raise ValueError('Registry must contain a counts object')
    ...
    if receipt is not None:
        verify_inventory(path, receipt)
        if receipt.get('dataset_hash') != digest:
            raise ValueError('Dataset digest differs from transfer receipt')
