...
function render(){
 ...
 try{
  ...
  }else if(!(tab==='llm'&&$('#llmWorkspace'))) $('#content').innerHTML=html;
...
async function refresh(redraw=true){if(refreshing)return;refreshing=true;try{[state,overview]=await Promise.all([api('/api/state'),api('/api/admin/overview')]);if(!filterDate)filterDate=state.today;if(tab==='llm')window.RoomLLM?.refresh();if(redraw)render();else{refreshTaskReadouts();syncPairPanel();$('#previewFrame')?.contentWindow.postMessage(demo?{type:'room-demo-state',state}:{type:'no-op'},'*')}}catch(e){if(e.status===401)showLogin();else R.toast(e.message)}finally{refreshing=false}}
