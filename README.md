...
자동 모드의 새 요청에서 날짜·제목·시각·대상이 빠진 경우, 관리자 요청 상세에서
그 값 하나를 이어서 입력할 수 있습니다. 180초/최대 6회, 명시적 부모 요청과 동일
관리자 세션에 묶이며 일반 음성/텍스트 입력을 자동 연결하지 않습니다. 슬롯을 채워도
기존 관리자 확인 전에는 비즈니스 데이터를 변경하지 않습니다. 최초 해석은 shadow
관측만 추가하며 Semantic Parser 1.2.0과 기존 operation 계약은 유지합니다.

설계·보안 경계·API·검증: [Classic NLU M3.3-A](docs/CLASSIC_NLU_M33.md).

### M3.3-B: 이름으로 메모 대상 확인

`포장 정리 메모 읽어줘`처럼 제목을 명시한 요청은 현재 저장된 메타데이터 목록에서
정확히 일치하거나 유일하게 부분 일치하는 메모를 찾습니다. 제목 중복·누락·잘림은
추측하지 않으며, 현재/최근 메모 선택은 기존 규칙을 유지합니다. 메모 본문 비우기·
덧붙이기·교체는 여전히 관리자 확인 후에만 실행합니다. 카탈로그와 개인 메모는
모델에 전달하지 않고, 공유 화면은 기존 비공개/버전 재검사를 거칩니다.
[구현 범위·검증·V35 적용](docs/ENTITY_CATALOG_M33B.md).
